...
const Link = artifacts.require("Token");
...
contract("Dex", (accounts) => {
  ...
  it("Market orders should not fill more limit orders than the market order amount", async () => {
    ...
    await dex.createLimitOrder(web3.utils.fromUtf8("LINK"), 1, 5, 300, {
      from: accounts[1],
    });
    await dex.createLimitOrder(web3.utils.fromUtf8("LINK"), 1, 5, 400, {
      from: accounts[2],
    });
    await dex.createLimitOrder(web3.utils.fromUtf8("LINK"), 1, 5, 500, {
      from: accounts[3],
    });
  ...
  it("Should throw an error when creating a buy market order without adequate ETH balance", async () => {
    ...
    await truffleAssert.reverts(
      dex.createMarketOrder(web3.utils.fromUtf8("LINK"), 0, 5, {
        from: accounts[4],
      })
    );
  });
